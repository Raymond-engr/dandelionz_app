...
const CategorySlider: React.FC<{ categories: Category[] }> = ({ categories: hardcodedCategories }) => {
  ...
  // Merge categories: hardcoded first, then unique API categories
  const allCategories = useMemo(() => {
    const hardcodedNames = new Set(hardcodedCategories.map(c => c.name.toLowerCase()));
    
    const uniqueApiCategories = apiCategories
      .filter((apiCat: any) => !hardcodedNames.has(apiCat.name.toLowerCase()))
      .map((apiCat: any) => ({
        id: apiCat.id,
        name: apiCat.name,
        image: apiCat.image // API categories should already have correct image URLs
      }));

    return [...hardcodedCategories, ...uniqueApiCategories];
